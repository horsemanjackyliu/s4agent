"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.entityCodeSample = entityCodeSample;
exports.operationCodeSample = operationCodeSample;
const util_1 = require("@sap-cloud-sdk/util");
const voca_1 = __importDefault(require("voca"));
const generator_without_ts_morph_1 = require("../generator-without-ts-morph");
const code_sample_util_1 = require("./code-sample-util");
/**
 * @internal
 */
function entityCodeSample(entityName, serviceName, directoryName) {
    return (0, util_1.codeBlock) `
import { ${voca_1.default.decapitalize(serviceName)} } from './generated/${directoryName}';

const { ${(0, generator_without_ts_morph_1.getApiName)(entityName)} } = ${voca_1.default.decapitalize(serviceName)}();
const resultPromise = ${(0, generator_without_ts_morph_1.getApiName)(entityName)}.requestBuilder().getAll().top(5).execute({ destinationName: 'myDestinationName' });

`;
}
/**
 * @internal
 */
function operationCodeSample(operation, packageName) {
    return importsCodeSample(operation, packageName);
}
function importsCodeSample(actionFunctionImport, packageName) {
    return (0, util_1.codeBlock) `
import { ${actionFunctionImport.name}${actionFunctionImport.parametersTypeName
        ? `, ${actionFunctionImport.parametersTypeName}`
        : ''} } from './generated/${packageName}';

${getParameterCodeSample(actionFunctionImport)}
const resultPromise = ${actionFunctionImport.name}(${actionFunctionImport.parametersTypeName ? 'parameter' : ''}).execute({ destinationName: 'myDestinationName' });
`;
}
function getParameterCodeSample(functionImport) {
    if (functionImport.parameters) {
        return `const parameter: ${functionImport.parametersTypeName} = ${(0, code_sample_util_1.getOperationParams)(functionImport.parameters)};
    `;
    }
    return '';
}
//# sourceMappingURL=code-samples.js.map