"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getApiSpecificUsage = getApiSpecificUsage;
const code_samples_1 = require("./code-samples");
const code_sample_util_1 = require("./code-sample-util");
/**
 * @internal
 */
function getApiSpecificUsage(service) {
    if (service.entities?.length > 0) {
        const entity = (0, code_sample_util_1.getODataEntity)(service.originalFileName, service.entities);
        return (0, code_samples_1.entityCodeSample)(entity.className, service.className, service.serviceOptions.directoryName);
    }
    // Return function/action import usage if no entity is found.
    if (service.operations.length > 0) {
        const operation = (0, code_sample_util_1.sampleOperation)(service.originalFileName, service.operations);
        return (0, code_samples_1.operationCodeSample)(operation, `${service.serviceOptions.directoryName}/operations`);
    }
    return '';
}
//# sourceMappingURL=generation-and-usage.js.map